...
export function Sidebar({
  ...
  const NavSection = ({
    ...
    <div className="mb-1">
      <p className="text-[9.5px] font-mono font-bold uppercase tracking-[1.2px] text-[#3e4155] px-3 py-2.5">
        {title}
      </p>
      {items.map((item) => (
        <button
        ...
        >
          <span
            className={active === item.id ? "text-[#00e5a0]" : "text-[#3e4155]"}
          >
            {item.icon}
          </span>
          {item.label}
    ...
    <div ref={itemsRef} className="flex flex-col h-full px-2 py-3">
      ...
      <div className="px-3 py-3 mb-3 border-b border-[rgba(255,255,255,0.06)]">
        <div className="flex items-center gap-2.5">
          <div
            className="w-9 h-9 rounded-full flex items-center justify-center text-[13px] font-bold text-[#050708] flex-shrink-0"
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10.5px] font-mono text-[#8b8fa8]">
              {student?.id || "---"}
            </p>
          </div>
        </div>
  ...
  return (
    <>
      ...
      <aside
        ref={sidebarRef}
        className="hidden lg:flex w-[200px] flex-shrink-0 bg-[#0f1117] border-r border-[rgba(255,255,255,0.06)] flex-col overflow-y-auto"
      >
        {sidebarContent}
      </aside>
