        ...
        {(["all", "success", "pending", "failed"] as const).map((f) => (
          ...
          >
            {f === "all"
              ...
              : f === "success"
                ? `Minted (${transactions.filter((t) => t.status === "success").length})`
                : f === "pending"
      ...
      <Card>
        ...
        <div ref={listRef} className="divide-y divide-[rgba(255,255,255,0.04)]">
          {filtered.map((tx) => (
            <div
              key={tx.id}
              className="tx-row grid grid-cols-1 md:grid-cols-[auto_1fr_auto_auto_auto] gap-2 md:gap-4 px-5 py-4 hover:bg-[rgba(255,255,255,0.02)] transition-colors items-center"
            >
              ...
              <div>
                ...
                {tx.ipfsCid && (
                  <p className="text-[10.5px] font-mono text-[#3e4155] mt-0.5">
                    IPFS: {tx.ipfsCid}
                  </p>
                )}
              </div>
