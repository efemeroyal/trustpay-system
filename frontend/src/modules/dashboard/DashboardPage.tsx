...
import {
  TrendingUp, ShieldCheck, Clock, Zap,
  History, Activity, LayoutGrid, ExternalLink,
...
export function DashboardPage() {
  ...
  const { transactions, receipts, dashboardStats, mintSteps, mintStatus } = usePayment()
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Greeting */}
      <div ref={greetingRef} className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
        ...
        {student?.walletAddress ? (
          <div className="flex items-center gap-2.5 bg-[rgba(0,229,160,0.06)] border border-[rgba(0,229,160,0.15)] rounded-xl px-4 py-2.5 self-start">
            ...
            <div>
              <p className="text-[9.5px] font-mono uppercase tracking-wide text-[#8b8fa8]">Wallet Connected</p>
              <p className="text-[12px] font-mono text-[#00e5a0]">{shortHash(student.walletAddress, 6)}</p>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-2.5 bg-[rgba(245,166,35,0.06)] border border-[rgba(245,166,35,0.2)] rounded-xl px-4 py-2.5 self-start">
            ...
            <p className="text-[12px] font-mono text-[#f5a623]">Connect wallet to receive SBTs</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          label="Total Paid" value={`₣ ${formatXAF(dashboardStats.totalPaid)}`}
          subtext="This academic session" accent="green"
          icon={<TrendingUp className="w-4 h-4" />} delay={0.05}
        />
        <StatCard
          label="SBT Receipts" value={dashboardStats.sbtCount.toString()}
          subtext="On Polygon network" accent="blue"
          icon={<ShieldCheck className="w-4 h-4" />} delay={0.1}
        />
        <StatCard
          label="Pending" value={dashboardStats.pendingCount.toString()}
          subtext={dashboardStats.pendingCount > 0 ? 'Awaiting confirmation' : 'All clear'}
          accent="amber"
          icon={<Clock className="w-4 h-4" />} delay={0.15}
        />
        <StatCard
          label="Gas Cost" value="₣ 0.00"
          subtext="Polygon = near-zero fees" accent="green"
          icon={<Zap className="w-4 h-4" />} delay={0.2}
        />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-4">
        ...
        <div className="flex flex-col gap-4">
          <Card>
            <CardHeader>
              <CardTitle icon={<History className="w-4 h-4" />}>Recent Transactions</CardTitle>
        ...
        <div className="flex flex-col gap-4">
          ...
          {pendingTx && (
            <Card>
              <CardHeader>
                <CardTitle icon={<Activity className="w-4 h-4" />}>Minting Status</CardTitle>
              ...
              <CardBody className="flex flex-col gap-0">
                <p className="text-[11.5px] font-mono text-[#8b8fa8] mb-3">
                  {pendingTx.feeLabel} · ₣ {formatXAF(pendingTx.amount)}
                </p>
                  ...
                  <div key={step.id} className="flex items-start gap-2.5">
                    <div className="flex flex-col items-center flex-shrink-0">
                      <div className={cn(
                        ...
                        step.status === 'done'   ? 'bg-[rgba(0,229,160,0.12)] text-[#00e5a0]' :
                        step.status === 'active' ? 'bg-[rgba(245,166,35,0.12)] text-[#f5a623]' :
          ...
          {receipts[0] && (
            <Card>
              <CardHeader>
                <CardTitle icon={<ShieldCheck className="w-4 h-4" />}>Latest Receipt</CardTitle>
                <Badge variant="success" dot>Soulbound</Badge>
              </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle icon={<LayoutGrid className="w-4 h-4" />}>Chain Activity</CardTitle>
              <span className="text-[10.5px] font-mono text-[#8b8fa8]">Amoy Testnet</span>
            </CardHeader>
            <div className="divide-y divide-[rgba(255,255,255,0.04)]">
              {[
                { type: 'MINT',   hash: '0x7da3fc28d91e45a8b312c4f09', block: 58204119 },
                { type: 'VERIFY', hash: '0x22cd9f47ae3b19fa0cde7bcf',  block: 58204071 },
                { type: 'MINT',   hash: '0x9fc4aab31c2017e38decd1bf',  block: 58203988 },
                { type: 'FAIL',   hash: '0xae01be445f7743ffdc3e18c8',  block: 58203901 },
              ].map((e, i) => (
                <div key={i} className="flex items-center gap-3 px-5 py-2.5">
                  <span className={cn(
                    'text-[10px] font-mono px-1.5 py-0.5 rounded flex-shrink-0',
                    e.type === 'MINT'   ? 'bg-[rgba(0,229,160,0.10)] text-[#00e5a0]' :
                    e.type === 'VERIFY' ? 'bg-[rgba(79,156,249,0.10)] text-[#4f9cf9]' :
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3 hover:bg-[rgba(255,255,255,0.02)] transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10.5px] font-mono text-[#8b8fa8] truncate">
          {tx.txHash
            ...
            : tx.status === 'pending' ? 'Processing on Polygon…' : 'Transaction failed'}
        </p>
      </div>
      <div className="text-right flex-shrink-0">
        ...
        <p className="text-[10.5px] text-[#3e4155] mt-0.5">{timeAgo(tx.timestamp)}</p>
      </div>
    </div>
  )
}
...
function SBTMiniCard({ receipt }: { receipt: SBTReceipt }) {
  return (
    <div
      ...
      style={{ background: 'linear-gradient(135deg, #0d1a14 0%, #0a1220 50%, #120e1a 100%)' }}
    >
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-[13px] font-semibold mt-0.5 leading-tight">{receipt.feeLabel}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2.5 mb-3">
          {([
            ...
            ['Amount', `₣ ${formatXAF(receipt.amount)}`],
            ['Token',  `#${receipt.tokenId}`],
            ['Minted', formatDate(receipt.mintedAt)],
          ] as [string, string][]).map(([k, v]) => (
            <div key={k}>
              <p className="text-[9px] font-mono uppercase tracking-wide text-[#8b8fa8]">{k}</p>
              <p
        ...
        <div className="flex items-center justify-between pt-2.5 border-t border-[rgba(255,255,255,0.06)]">
          ...
          <a href="#" className="flex items-center gap-1 text-[10.5px] font-mono text-[#4f9cf9] hover:underline">
