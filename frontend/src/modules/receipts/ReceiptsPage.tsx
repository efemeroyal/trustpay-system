...
function getReceiptState(receipt: ReceiptType) {
  if (
    receipt.verificationStatus === "validated" ||
    receipt.isVerified ||
    receipt.sbt?.mintStatus === "minted"
  ) {
    return "validated";
  }
  if (
    receipt.verificationStatus === "rejected" ||
    receipt.sbt?.mintStatus === "failed"
  ) {
    return "rejected";
  }
  return "pending";
...
export function ReceiptsPage() {
  ...
  const filtered = receipts.filter((r) => {
    const state = getReceiptState(r);
    if (filter === "all") return true;
    if (filter === "verified") return state === "validated";
    if (filter === "pending") return state === "pending";
    if (filter === "failed") return state === "rejected";
    return true;
  });

  const counts = {
    all: receipts.length,
    verified: receipts.filter((r) => getReceiptState(r) === "validated").length,
    pending: receipts.filter((r) => getReceiptState(r) === "pending").length,
    failed: receipts.filter((r) => getReceiptState(r) === "rejected").length,
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[#8b8fa8] mt-0.5">
            {receipts.length} total payment{receipts.length !== 1 ? "s" : ""}
          </p>
        </div>
      </div>
...
function ReceiptRow({
  ...
  const receiptState = getReceiptState(receipt);
  ...
  const statusIcon = () => {
    if (receiptState === "validated")
      return <ShieldCheck className="w-4 h-4 text-[#00e5a0]" />;
    if (receiptState === "rejected")
      return <XCircle className="w-4 h-4 text-[#ff5757]" />;
    ...
    <div className="receipt-row">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10.5px] text-[#8b8fa8] mt-0.5 font-mono">
            {receipt.referenceNumber}
          </p>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="text-right hidden sm:block">
            <p className="text-[13px] font-mono font-bold text-[#00e5a0]">
              {formatXAF(receipt.amount)} CFA
            </p>
            <p className="text-[10.5px] text-[#3e4155]">
          ...
          <ChevronDown
            className={cn(
              "w-4 h-4 text-[#3e4155] transition-transform duration-200",
      ...
      >
        <div className="px-5 pb-5">
          <div className="bg-[#0f1420] rounded-xl p-4 border border-[rgba(255,255,255,0.06)]">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              {receipt.qrCodeData && (
                <div className="flex-shrink-0 flex flex-col items-center gap-2">
                  ...
                  />
                  <p className="text-[9.5px] font-mono text-[#3e4155] text-center">
                    Scan to verify
                  </p>
                </div>
              )}
              ...
              <div className="flex-1 grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={k}>
                    ...
                    <p className="text-[12px] font-medium text-[#e8eaf0] mt-0.5 capitalize">
                      {v}
                    </p>
                  </div>
                ))}
            ...
            {receiptState === "rejected" && receipt.rejectionReason && (
              <div className="mt-4 pt-4 border-t border-[rgba(255,255,255,0.06)]">
                <p className="text-[12px] text-[#ff5757] font-medium">
                  {receipt.rejectionReason}
                </p>
              </div>
            )}

            {receiptState === "validated" && receipt.sbt?.transactionHash && (
              <div className="mt-4 pt-4 border-t border-[rgba(255,255,255,0.06)]">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    <ShieldCheck className="w-3.5 h-3.5 text-[#00e5a0]" />
                    <span className="text-[12px] text-[#00e5a0] font-medium">
                      Receipt ID: #{receipt.sbt.tokenId}
                    </span>
                  </div>
                  <a
