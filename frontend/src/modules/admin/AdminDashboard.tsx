  ...
  const handleSearch = async () => {
    ...
    const matches = receipts.filter((r) =>
      mode === "student"
        ...
        : r.contractAddress.toLowerCase().includes(query.toLowerCase()) ||
          query.startsWith("0x"),
    );
    ...
    <div className="flex flex-col gap-5">
      <div>
        ...
        <p className="text-sm text-[#8b8fa8] mt-0.5">
          Verify student payment receipts via blockchain query
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Total Volume",
            value: `${formatXAF(receipts.reduce((a, r) => a + r.amount, 0))} CFA`,
            color: "#f5a623",
          },
          {
            label: "Contract",
            value: shortAddress("0x742d35Cc6634C0532925a3b"),
            color: "#8b8fa8",
          },
        ].map((s) => (
          <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <Card>
          ...
          <CardBody className="flex flex-col gap-4">
            ...
            <Input
              placeholder={
                mode === "student" ? "e.g. UB22CS041" : "e.g. 0x3f9a…c7d2"
              }
              value={query}
            ...
            <div className="flex items-center gap-2 text-xs text-[#8b8fa8] bg-[#161921] rounded-lg p-3">
              ...
              <span>
                You can also scan the QR code on the student's SBT receipt card
                to verify instantly.
              </span>
            </div>
          </CardBody>
          ...
          <CardHeader>
            ...
            {result !== null && (
              <Badge variant={result.length > 0 ? "success" : "failed"} dot>
                {result.length > 0
                  ? `${result.length} receipt${result.length > 1 ? "s" : ""} found`
                  : "Not found"}
              </Badge>
            )}
          </CardHeader>
          <CardBody>
            ...
            {isSearching && (
              <div className="flex flex-col items-center justify-center py-10 gap-3">
                ...
                <p className="text-sm font-mono text-[#8b8fa8]">
              ...
              <div ref={resultRef} className="flex flex-col gap-3">
                {result.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-8">
                    ...
                    <p className="text-sm text-[#8b8fa8] font-mono">
                  ...
                  result.map((r) => (
                    <div
                      key={r.tokenId}
                      className="bg-[#161921] rounded-xl p-4 border border-[rgba(0,229,160,0.12)]"
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <p className="text-[10.5px] font-mono uppercase tracking-wide text-[#8b8fa8]">
                            Receipt #{r.tokenId}
                          </p>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11.5px]">
                        {[
                          ...
                          <div key={k}>
                            <p className="text-[#8b8fa8] font-mono text-[10px] uppercase tracking-wide">
                              {k}
                            </p>
            ...
            <thead>
              <tr className="border-b border-[rgba(255,255,255,0.06)]">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-5 py-2.5 text-[10px] font-mono uppercase tracking-wide text-[#3e4155]"
                  >
                    {h}
                  </th>
                ))}
              ...
              {receipts.map((r, i) => (
                ...
                >
                  ...
                  <td className="px-5 py-3 font-mono text-[#00e5a0]">
                    {formatXAF(r.amount)} CFA
                  </td>
                  <td className="px-5 py-3 font-mono text-[#8b8fa8]">
                    {formatDate(r.mintedAt)}
                  </td>
                  <td className="px-5 py-3 font-mono text-[#4f9cf9]">
